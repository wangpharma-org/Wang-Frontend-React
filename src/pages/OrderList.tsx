...
const OrderList = () => {
  ...
  useEffect(() => {
    ...
    newSocket.on("listorder:get", (data) => {
      ...
      if (data.tracking && data.tracking.expires_at >= Date.now()) {
        const visibleShRunning = new Set(
          nextOrders.flatMap((member) => member.all_sh_running)
        );

        if (!visibleShRunning.has(data.tracking.sh_running)) {
          newSocket.emit("listorder:client-missing", {
            event_id: data.tracking.event_id,
            sh_running: data.tracking.sh_running,
            mem_code: data.tracking.mem_code,
